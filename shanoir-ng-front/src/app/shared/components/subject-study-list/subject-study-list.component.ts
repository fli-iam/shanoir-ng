...
export class SubjectStudyListComponent extends AbstractInput<Subject[]> implements OnChanges, OnDestroy {
    ...
    removeSubjectOk(subject: Subject):void {
        ...
        if (index > -1) {
            ...
            this.propagateChange(this.model);
            if (this.compMode == 'study') {
                let option: Option<Subject> = this.optionList.find(opt => opt.value.id == subject.id) as Option<Subject>;
                if (option) option.disabled = false;
            } else if (this.compMode == 'subject') {
                let option: Option<Study> = this.optionList.find(opt => opt.value.id == subject.study.id) as Option<Study>;
                if (option) option.disabled = false;
            }
        }
        this.table.refresh();
