...
export class AnestheticsListComponent  extends BrowserPaginEntityListComponent<Anesthetic>{
    ...
    protected openDeleteConfirmDialog = (entity: Anesthetic) => {
        ...
        this.examinationAnestheticService.getAllExaminationForAnesthetic(entity.id)
            .then(examinationAnesthetics => {
                if (examinationAnesthetics?.length > 0) {
                    this.confirmDialogService.confirm(
                        'Delete anesthetic',
                        'This anesthetic is linked to preclinical examinations, it can not be deleted'
                    );
                } else {
                    confirmAndDelete();
                }
            })
            .catch(() => confirmAndDelete());
    }
}
