...
public class KeycloakInitServer extends AbstractKeycloakInit
{
	...
	private static void createAuthenticationFlow() {
		...
		final AuthenticationManagementResource authenticationManagement = getKeycloak().realm(getKeycloakRealm()).flows();
		...
		final List<AuthenticationExecutionInfoRepresentation> exeutions = authenticationManagement.getExecutions(NEW_BROWSER_FLOW);
		...
		response = getKeycloak().realm(getKeycloakRealm()).flows().newExecutionConfig(executionInfo.getId(), authenticatorConfig);
		if (response.getStatus() != 201) {
	...
	private static void updateRequiredAction() {
		final List<RequiredActionProviderRepresentation> requiredActions = getKeycloak().realm(getKeycloakRealm()).flows().getRequiredActions();
