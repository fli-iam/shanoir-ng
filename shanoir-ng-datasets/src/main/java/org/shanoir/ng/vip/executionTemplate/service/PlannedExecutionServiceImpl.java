...
public class PlannedExecutionServiceImpl implements PlannedExecutionService {
    ...
    public List<Long> getInvolvedData(ExecutionInQueue execution) {
        switch (execution.getType()) {
            case "dataset" -> {
                return List.of(execution.getObjectId());
            }
            case "acquisition" -> {
                return datasetRepository.findFilteredIdsByDatasetAcquisitionId(execution.getObjectId(), "");
            }
            case "examination" -> {
                return datasetRepository.findIdsByExaminationId(execution.getObjectId());
            }
            default -> {
                return null;
            }
