 ...
 */
@Controller
public class ExecutionTemplateFilterApiController implements ExecutionTemplateFilterApi {

    @Autowired
    private ExecutionTemplateFilterService service;
    ...
    public ResponseEntity<List<ExecutionTemplateFilterDTO>> getExecutionTemplateFiltersByExecutionTemplateId(@Parameter(description = "The execution template Id", required = true) @PathVariable("executionTemplateId") Long executionTemplateId) {
        List<ExecutionTemplateFilter> filters = repository.findByExecutionTemplateId(executionTemplateId);
