...
public interface ExecutionDataApi {
    ...
    ResponseEntity<?> createExecution(
            @Parameter(description = "execution", required = true) @RequestBody final String execution) throws EntityNotFoundException, SecurityException;
    ...
    @RequestMapping(value = "/execution/{identifier}",
            produces = { "application/json", "application/octet-stream" },
            method = RequestMethod.GET)
    ResponseEntity<String> getexecutionStatus(@Parameter(description = "The execution identifier", required=true) @PathVariable("identifier") String identifier) throws IOException, RestServiceException, EntityNotFoundException, SecurityException;
