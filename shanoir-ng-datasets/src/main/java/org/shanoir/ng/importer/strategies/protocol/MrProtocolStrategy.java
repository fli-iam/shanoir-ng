...
/**
 * MK: validated, multi-threading safe.
 * 
 * @author mkain
 *
 */
...
public class MrProtocolStrategy implements ProtocolStrategy {
	...
	public MrProtocol generateMrProtocolForSerie(Attributes attributes, Serie serie) {
		// this is not available as Spring component, so we create it for each method
		// call as I took not the time to analyse MfE on multi-threading capacities,
		// better in a multi-threading web env not taking a risk here
		...
		// Number of averages
		final Integer numberOfAverages = getNumberOfAverages(attributes, serie.getIsEnhancedMR());
		...
		if (serie.getIsEnhancedMR()) {
			...
			final String inPlanePhaseEncodingDirection = getInPlanePhaseEncodingDirection(attributes,
					serie.getIsEnhancedMR());
			if (inPlanePhaseEncodingDirection != null) {
				final Integer mRAcquisitionFrequencyEncodingSteps = getMRAcquisitionFrequencyEncodingSteps(attributes,
						serie.getIsEnhancedMR());
				if (getInPlanePhaseEncodingDirection(attributes, serie.getIsEnhancedMR()).equals("ROW")) {
				...
				} else if (getInPlanePhaseEncodingDirection(attributes, serie.getIsEnhancedMR()).equals("COLUMN")) {
		...
		// TODO ATO : Add preclinical mode condition. if preclinical then implement
		// DicomMetadataExtractor.extractMetadataCompletingMrProtocol(..) method in
		// shanoir old
	...
	private MrProtocolMetadata createOriginMrProtocolMetadata(Attributes dicomAttributes, Serie serie) {
		...
		/*
		 * Mr Sequence Application, RefMrSequenceKSpaceFill, Slice Order, Contrast
		 * agent, Slice orientation at acquisition, Acquisition Contrast, MR Sequence
		 * Physics -> to be entered later with the study card mechanism
		 */
		...
		final Double contrastAgentConcentration = dicomAttributes.getDouble(Tag.ContrastBolusIngredientConcentration,
				0);
		...
		final String parallelAcquisitionTechniqueExtracted = dicomAttributes
		...
		final Double timeReductionFactorForTheInPlaneDirection = dicomAttributes
		...
		final Double timeReductionFactorForTheOutOfPlaneDirection = dicomAttributes
		...
		// TODO Fix this using either field : (0018,0021) check if MT value in sequence
		// (cf http://dicomlookup.com/lookup.asp?sw=Tnumber&q=(0018,0021) )
		// OR use Tag.MagnetizationTransfer (cf
		// http://dicomlookup.com/lookup.asp?sw=Tnumber&q=(0018,9020) ) possible values:
		// ON_RESONANCE OFF_RESONANCE NONE
		//
	...
	public String getReceiveCoilName(final Attributes dicomAttributes, final boolean isEnhancedMR) {
		...
		if (result != null && !"".equals(result)) {
			...
			final Object obj = getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING,
					Tag.ReceiveCoilName, dicomAttributes);
			if (obj != null) {
				return (String) obj;
			} else {
				return null;
	...
	public String getReceiveCoilType(final Attributes dicomAttributes, final boolean isEnhancedMR) {
		...
		if (result != null && !"".equals(result)) {
			...
			final Object obj = getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING,
					Tag.ReceiveCoilType, dicomAttributes);
			if (obj != null) {
				return (String) obj;
			} else {
				return null;
	...
	public String getTransmitCoilName(final Attributes dicomAttributes, final boolean isEnhancedMR) {
		...
		if (result != null && !"".equals(result)) {
			...
			final Object obj = getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING,
					Tag.TransmitCoilName, dicomAttributes);
			if (obj != null) {
				return (String) obj;
			} else {
				return null;
	...
	public String getTransmitCoilType(final Attributes dicomAttributes, final boolean isEnhancedMR) {
		...
		if (result != null && !"".equals(result)) {
			...
			final Object obj = getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING,
					Tag.TransmitCoilType, dicomAttributes);
			if (obj != null) {
				return (String) obj;
			} else {
				return null;
	...
	/**
	 * Depending on the type given in parameter, call the correct method to get the
	 * value of the dicom tag.
	 *
	 * @param type
	 ...
	 */
	public static Object getValue(final ShanoirConstants.DICOM_RETURNED_TYPES type, final int dicomTag,
	...
	public Object getValueInSharedAndThenPerFrame(final ShanoirConstants.DICOM_RETURNED_TYPES type,
			final int dicomTagPath, final Attributes dicomAttributes) {
//		Object result;
...
//		result = getValue(type, newTagPath, dcmObj);
//		if (result != null) {
//			return result;
//		} else {
...
//		}
	...
	public String getInPlanePhaseEncodingDirection(final Attributes dicomAttributes, final boolean isEnhancedMR) {
		if (!isEnhancedMR) {
			...
			final Object obj = getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING,
					Tag.InPlanePhaseEncodingDirection, dicomAttributes);
			if (obj != null) {
				return (String) obj;
			} else {
				return null;
	...
	public Integer getEchoTrainLength(final Attributes dicomAttributes, final boolean isEnhancedMR) {
		if (!isEnhancedMR) {
			...
			final Object obj = getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING,
					Tag.EchoTrainLength, dicomAttributes);
			if (obj != null) {
	...
	public Integer getMRAcquisitionFrequencyEncodingSteps(final Attributes dicomAttributes,
			final boolean isEnhancedMR) {
		if (!isEnhancedMR) {
			...
			final Object obj = getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING,
					Tag.MRAcquisitionFrequencyEncodingSteps, dicomAttributes);
			if (obj != null) {
	...
	/**
	 * Method copied from sh-old: to clarify why such a strange conversion from String
	 * to Double to Integer has been chosen.
	 * @param attributes
	 * @param isEnhancedMR
	 * @return
	 */
	public Integer getNumberOfAverages(final Attributes attributes, final boolean isEnhancedMR) {
		String numberOfAverages = null;
		if (!isEnhancedMR) {
			numberOfAverages = attributes.getString(Tag.NumberOfAverages);
		} else {
			int[] tagPath = new int[3];
			tagPath[0] = Tag.MRAveragesSequence;
			tagPath[2] = Tag.NumberOfAverages;
//			numberOfAverages = (String) getValueInSharedAndThenPerFrame(ShanoirConstants.DICOM_RETURNED_TYPES.STRING, tagPath,
//					attributes);
		}
