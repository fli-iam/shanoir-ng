...
public class DatasetAcquisitionApiController implements DatasetAcquisitionApi {
	...
	public ResponseEntity<List<ExaminationDatasetAcquisitionDTO>> findDatasetAcquisitionByExaminationId(Long examinationId) {
		...
		LOG.error("######### - findByExamination took {} ms", System.currentTimeMillis() - ts);
		...
		LOG.error("######### - sorting took {} ms", System.currentTimeMillis() - ts);
		if (daList.isEmpty()) {
			return new ResponseEntity<>(HttpStatus.NO_CONTENT);
		} else {
			ts = System.currentTimeMillis();
			List<ExaminationDatasetAcquisitionDTO> dtoList = examDsAcqMapper.datasetAcquisitionsToExaminationDatasetAcquisitionDTOs(daList);
			LOG.error("######### - mapping took {} ms", System.currentTimeMillis() - ts);
