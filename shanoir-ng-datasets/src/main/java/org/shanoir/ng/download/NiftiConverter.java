...
public enum NiftiConverter {
	...
	NiftiConverter(final int id, final String path) {
		this.id = id;
		this.path = path;
	}
	...
	public static Integer getId(String type){
		...
		for (NiftiConverter converter : NiftiConverter.values()) {
			if (NiftiConverter.valueOf(type).equals(converter)) {
				return converter.getId();
			}
		}
		throw new IllegalArgumentException("No matching type for type " + type);
	}
}
