...
public class DicomJsonUtils {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DicomJsonUtils.class);
...
//        LOG.error(studies);
        ...
        for (int i = 0; i < studies.length(); i++) {
            ...
            for (int j = 0; j < series.length(); j++) {
                ...
                for (int k = 0; k < instances.length(); k++) {
                    JSONObject instance = instances.getJSONObject(k);
                    cleanChildWithParent(serie, instance);
                }
                cleanChildWithParent(study, serie);
            }
        }
        LOG.error("#########################################################");
        LOG.error(studies.toString());
    ...
    private static void cleanChildWithParent(JSONObject parent, JSONObject child) throws JSONException {
        JSONArray properties = child.names();
        for (int l = 0; l < properties.length(); l++) {
            String propertyName = properties.getString(l);
            if (parent.has(propertyName)) {
