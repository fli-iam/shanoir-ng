...
public class DatasetServiceSecurityTest {
	...
	private void testCreate() throws ShanoirException {
		...
		mrDs.getDatasetAcquisition().getExamination().setStudyId(1L);
		...
		assertAccessDenied(service::create, mrDs);
		mrDs.getDatasetAcquisition().getExamination().setStudyId(2L);
		assertAccessDenied(service::create, mrDs);
		mrDs.getDatasetAcquisition().getExamination().setStudyId(3L);
		assertAccessDenied(service::create, mrDs);
	}
	...
	private void setCenterRightsContext() {
		...
		given(datasetRepository.findByDatasetAcquisitionExaminationStudyId(1L)).willReturn(new PageImpl<>((Arrays.asList(new Dataset[]{dataset1, dataset3}))));
