...
public class NIfTIConverterService {
	...
	public void prepareAndRunConversion(Patient patient, File workFolder, Long converterId)
		...
		if (!seriesFolderFile.exists()) {
			...
			throw new RestServiceException(new ErrorModel(HttpStatus.UNPROCESSABLE_ENTITY.value(),
					"Error while creating series folder: folder already exists.", null));
		}
		...
		for (Iterator<Study> studiesIt = studies.iterator(); studiesIt.hasNext();) {
			...
			for (Iterator<Serie> seriesIt = series.iterator(); seriesIt.hasNext();) {
				...
				if (serie.getSelected()) {
					...
					try {
						serieIdentifiedForNotSeparating = checkSerieForPropertiesString(serie,
								doNotSeparateDatasetsInSerie);
	...
	private ExpressionFormat generateNiftiDatasetExpression(Dataset dataset, Serie serie) {
		LOG.debug("Create the nifti dataset expressions for dataset : " + dataset.getName() + " in serie "
				+ serie.getSequenceName());
	 ...
	 * 
	 **/
	private void generateNiftiDatasetFiles(ExpressionFormat datasetExpressionFormat, Dataset dataset,
			List<File> niftiFileList) {
	...
	/**
	 * Extract from the bvec and bval files the diffusion gradients and fullfill the
	 * mr protocol.
	 *
	 * @param mrProtocol
	 *            the mr protocol
	 ...
	 * @throws NoSuchFieldException
	 */
	private NIfTIConverter datasetToNiftiConversionLauncher(Dataset dataset, File directory, Serie serie,
			Long converterId, boolean isConvertAs4D, boolean isConvertWithClidcm)
			throws NoSuchFieldException, SecurityException {
	...
	private List<File> niftiFileSorting(Long converterId, List<File> existingFiles, File directory,
			File serieIDFolderFile) {
	...
	/**
	 * This method generates the nifti files of serie in proper datasets for an
			...
			if (serie.getDatasets().size() > 1) {
				...
				for (Dataset dataset : serie.getDatasets()) {
					...
					if (directory.isDirectory()) {
						LOG.info("convertToNifti : create nifti files for the dataset " + dataset.getName()
								+ " in directory : " + directory.getName());
						final List<File> existingFiles = Arrays.asList(directory.listFiles());
						NIfTIConverter converter = null;
						try {
							converter = datasetToNiftiConversionLauncher(dataset, directory, serie, converterId,
									isConvertAs4D, isConvertWithClidcm);
						} catch (NoSuchFieldException | SecurityException e) {
							LOG.error(e.getMessage());
						}
						List<File> niftiGeneratedFiles = niftiFileSorting(converterId, existingFiles, directory,
								serieIDFolderFile);
				...
				if (serieIDFolderFile.isDirectory()) {
					LOG.info("convertToNifti : create nifti files for the dataset " + dataset.getName()
							+ " in directory : " + serieIDFolderFile.getName());
					final List<File> existingFiles = Arrays.asList(serieIDFolderFile.listFiles());
					NIfTIConverter converter = null;
					try {
						converter = datasetToNiftiConversionLauncher(dataset, serieIDFolderFile, serie, converterId,
								isConvertAs4D, isConvertWithClidcm);
					} catch (NoSuchFieldException | SecurityException e) {
						LOG.error(e.getMessage());
					}
					List<File> niftiGeneratedFiles = niftiFileSorting(converterId, existingFiles, serieIDFolderFile,
							serieIDFolderFile);
					constructNiftiExpressionAndDatasetFiles(converter, dataset, serie, niftiGeneratedFiles);
				}
			}
	 ...
	 */
	private void constructNiftiExpressionAndDatasetFiles(NIfTIConverter converter, Dataset dataset, Serie serie,
			List<File> niftiGeneratedFiles) {
		// Build dataset Expresion and datasetFiles
		...
		if (!serieIdentifiedForNotSeparating) {
			...
			for (Image image : serie.getImages()) {
				...
				if (found) {
					...
					datasetMap.get(seriesToDatasetsSeparator).getRepetitionTimes()
							.add(new Double(image.getRepetitionTime()));
					datasetMap.get(seriesToDatasetsSeparator).getInversionTimes()
							.add(new Double(image.getInversionTime()));
			...
			for (final SerieToDatasetsSeparator datasets : datasetMap.keySet()) {
				...
				for (final DatasetFile datasetFile : datasetMap.get(datasets).getExpressionFormats().get(0)
						.getDatasetFiles()) {
	...
	private File createSerieIDFolder(File workFolder, File seriesFolderFile, Serie serie) throws RestServiceException {
		...
		if (!serieIDFolderFile.exists()) {
			...
			throw new RestServiceException(new ErrorModel(HttpStatus.UNPROCESSABLE_ENTITY.value(),
					"Error while creating serie id folder: folder already exists.", null));
		}
	...
	private void moveFiles(File workFolder, File serieIDFolder, List<Image> images) throws RestServiceException {
		for (Iterator<Image> iterator = images.iterator(); iterator.hasNext();) {
			...
			if (oldFile.exists()) {
				...
				throw new RestServiceException(new ErrorModel(HttpStatus.UNPROCESSABLE_ENTITY.value(),
						"Error while creating serie id folder: file to copy does not exist.", null));
			}
		}
	}
