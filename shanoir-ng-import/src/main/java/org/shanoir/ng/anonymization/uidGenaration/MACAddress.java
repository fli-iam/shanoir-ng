...
/**
 * <p>
 * A class for fetching the MAC address of one of the local host network
 * interfaces.
 * </p>
 *
 * <p>
...
public class MACAddress {
	...
	private static final String regexForMACSeparator = "[:-]"; // : on unix, -
																// on windoze
		...
		public void run() {
			try {
				...
				while ((line = r.readLine()) != null && (value == null || value.length() == 0)) {
					// System.err.println("MACAddress.ConsumeStreamLookingForRexEx.run():
					// read line=\""+line+"\"");
	...
	// executeCommandLookingForFirstLineContainingAndGetMatchingString(String[]
	// command,String regex) {
	private final String executeCommandLookingForFirstLineContainingAndGetMatchingString(String command, String regex) {
		// System.err.println("MACAddress.executeCommandLookingForFirstLineContaining():
		// command=\""+command+"\"");
		String value = null;
		try {
			...
			// System.err.println("MACAddress.executeCommandLookingForFirstLineContaining():
			// exitVal=\""+exitValue+"\"");
			// don't try to get a value until the output has been completely
			// processed (may be after process finished)
			outConsumer.join();
			errConsumer.join();
			...
			// ignore exception (e.g. if cannot find command on another
			// platform, such as "java.io.IOException: cmd: not found")
			// e.printStackTrace(System.err);
		}
		return value;
	}
	...
	private static final long extractMACAddressFromHexComponents(String macAddressString) {
		...
		// System.err.println("MACAddress.extractMACAddressFromHexComponents():
		// MAC address="+macAddressValue+" dec
		// (0x"+Long.toHexString(macAddressValue)+")");
		return macAddressValue;
	}
	...
	private static final long extractMACAddressFromByteArray(byte[] macAddressBytes) {
		...
		// System.err.println("MACAddress.extractMACAddressFromByteArray(): MAC
		// address="+macAddressValue+" dec
		// (0x"+Long.toHexString(macAddressValue)+")");
		return macAddressValue;
	}
		...
		try {
			...
			if (interfaces != null) {
				while (!foundOne && interfaces.hasMoreElements()) {
					...
					if (i != null) {
						// System.err.println("MACAddress.getMacAddressFromNetworkInterfaceAPI():
						// network interface name="+i.getName());
						Class[] argTypes = {};
						Object[] argValues = {};
						java.lang.reflect.Method getHardwareAddress = i.getClass()
								.getDeclaredMethod("getHardwareAddress", argTypes);
						// System.err.println("MACAddress.getMacAddressFromNetworkInterfaceAPI():
						// getHardwareAddress method="+getHardwareAddress);
						...
						if (macAddressBytes != null) {
							...
							// System.err.println("MACAddress.getMacAddressFromNetworkInterfaceAPI():
							// macAddressValue=0x"+Long.toHexString(macAddressValue));
							foundOne = true;
						}
					}
				}
	...
	protected long getMacAddressFromSystemCommandCall() {
		...
		// windows needs ipconfig, as well as the /all option, and a hyphen
		// rather than colon seperator
		// do NOT try Windows 95 command.com instead of cmd.exe - causes dialog
		// and error to popup
		if (macAddressString == null) {
			// System.err.println("MACAddress(): Try Windows NT/2000/XP
			// ipconfig");
			String cmd = "ipconfig /all";
			macAddressString = executeCommandLookingForFirstLineContainingAndGetMatchingString(cmd, regexForMAC);
			// System.err.println("MACAddress(): MAC address from \""+cmd+"\"
			// =\""+macAddressString+"\"");
		}
		if (macAddressString == null) {
		...
		// System.err.println("MACAddress(): MAC address from
		// ifconfig=\""+macAddressString+"\"");
		// a plain ifconfig will not work for solaris unless one is root, so get
		// it from arp (which does not work for linux or darwin)
		if (macAddressString == null) {
		...
		// System.err.println("MACAddress(): MAC address="+macAddressValue+" dec
		// (0x"+Long.toHexString(macAddressValue)+")");
		return macAddressValue;
	}
