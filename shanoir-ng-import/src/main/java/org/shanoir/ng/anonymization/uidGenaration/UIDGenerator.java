...
public class UIDGenerator {
	...
	private static final int maxStampComponentLength = 64 - root.length() - 1 // root
			...
			- 5 - 5 - 5; // . SOP Instance UID study# (4) . series# (4) .
							// instance# (4)
	...
	private static long machineAddress = vmid.isUnique() ? (((long) vmid.hashCode()) & 0x0ffffffffl)
			: new MACAddress().getMACAddress();
	 ...
	 *
	 * <p>
	 * This will use random and installation specific elements to create a
	 * unique root.
	 * </p>
	 *
	 */
	public UIDGenerator() {
		// System.err.println("UIDGenerator():
		// vmid.isUnique()="+vmid.isUnique());
		// System.err.println("UIDGenerator():
		// vmid.hashCode()="+vmid.hashCode());
	...
	/**
	 * <p>
	 * Reinitialize the UID generator with a new stamp using random and
	 * installation specific elements to create a unique root.
	 * </p>
	 *
	 * <p>
	...
	public void newStamp() {
		long ourMachine = Math.abs(machineAddress); // don't mess with the real
													// machine address; need it
													// unchanged for next time
		...
		int ourCount = Math.abs(Short.valueOf(st.nextToken(), 16).shortValue() + 0x8000); // why
																							// add
																							// 0x8000
																							// ?
																							// because
		...
		while (ourUnique > 10000 && machineString.length() + vmString.length() + timeString.length()
				+ countString.length() > maxLongStampComponentLength) {
			// System.err.println("stamp length > maximum which is
			// "+maxStampComponentLength+" shortening VM specific string");
			ourUnique = ourUnique / 10;
			vmString = Integer.toString(ourUnique);
		}
		while (ourMachine > 0 && machineString.length() + vmString.length() + timeString.length()
				+ countString.length() > maxLongStampComponentLength) {
			// System.err.println("stamp length > maximum which is
			// "+maxStampComponentLength+" shortening MAC specific string");
		...
		while (ourUnique > 10000 && machineString.length() + vmString.length() + timeString.length()
				+ countString.length() > maxStampComponentLength) {
			// System.err.println("stamp length > maximum which is
			// "+maxStampComponentLength+" shortening VM specific string");
			ourUnique = ourUnique / 10;
			vmString = Integer.toString(ourUnique);
	 ...
	 *
	 * <p>
	 * This will use the supplied stamp rather than generating a unique root, to
	 * create repeatable UIDs.
	 * </p>
	 *
	 * @param stamp
	...
	private String getLimitedLengthNumericPartOfStringOrZeroAsUIDExtension(String string) {
		...
		if (string != null) {
			try {
				...
				if (numericPart > 9999) {
					numericPart = numericPart % 10000; // no longer than 4
														// decimal digits
				}
	 ...
	 *
	 * <p>
	 * This will always be the same for this instance of the UIDGenerator,
	 * unless newStamp() has been called since the last time.
	 * </p>
	 *
	 * @return the UID
	 ...
	 *
	 * <p>
	 * This will be the same for this instance of the UIDGenerator and the same
	 * parameter values.
	 * </p>
	 *
	 * <p>
	 * Only use this if you really need reproducible UIDs; otherwise use
	 * getNewUID().
	 * </p>
	 *
	 * @param studyID
	...
	public String getNewStudyInstanceUID(String studyID) throws DicomException {
		String uid = root + "." + stamp + "." + UIDGEN_INSTANCE_STUDY
	...
	public String getNewSeriesInstanceUID(String studyID, String seriesNumber) throws DicomException {
		String uid = root + "." + stamp + "." + UIDGEN_INSTANCE_SERIES
				+ getLimitedLengthNumericPartOfStringOrZeroAsUIDExtension(studyID)
	...
	public String getNewSOPInstanceUID(String studyID, String seriesNumber, String instanceNumber)
		...
		String uid = root + "." + stamp + "." + UIDGEN_INSTANCE_SOP
				+ getLimitedLengthNumericPartOfStringOrZeroAsUIDExtension(studyID) // max
																					// length
																					// 5
				+ getLimitedLengthNumericPartOfStringOrZeroAsUIDExtension(seriesNumber) // max
																						// length
																						// 5
				+ getLimitedLengthNumericPartOfStringOrZeroAsUIDExtension(instanceNumber); // max
																							// length
																							// 5
	 ...
	 *
	 * @param arg
	 *            a single numeric argument that is the number of UIDs to
	 *            generate
	 */
