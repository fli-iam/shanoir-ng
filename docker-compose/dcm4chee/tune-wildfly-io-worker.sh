...
if [ -f "$CONF" ] && ! grep -q 'io-threads=' "$CONF"; then
    sed -i 's#<worker name="default"/>#<worker name="default" io-threads="${env.WILDFLY_IO_THREADS:32}" task-max-threads="${env.WILDFLY_IO_TASK_MAX_THREADS:1200}"/>#' "$CONF"
    if ! grep -q 'io-threads=' "$CONF"; then
        echo "tune-wildfly-io-worker.sh: WARNING - expected '<worker name=\"default\"/>' not found in $CONF, io-threads tuning was NOT applied" >&2
    fi
fi
