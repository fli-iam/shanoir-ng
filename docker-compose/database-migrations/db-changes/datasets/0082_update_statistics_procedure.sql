...
SELECT subject.id AS patient_id,
    ...
    dataset_acquisition.id AS acquisition_id,
    dataset.source_id AS source_id,
    ...
    'version', exe.pipeline_identifier,
    ...
    GROUP BY proc.id, exe.name, exe.status, proc.processing_date, exe.pipeline_identifier
    ) AS execution
FROM dataset
    LEFT JOIN dataset_acquisition ON (dataset_acquisition.id = dataset.dataset_acquisition_id)
    LEFT JOIN dataset_metadata ON (dataset_metadata.id = dataset.updated_metadata_id)
