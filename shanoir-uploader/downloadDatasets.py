...
parser.add_argument('-dsc', '--data_description', required=True, help='Path to the data description file (.xls).')
...
def get_filename_from_response(dest_dir, response):
    filename = None
    if response.headers and 'Content-Disposition' in response.headers:
        ...
        filename = str(dest_dir / filenames[0]) if len(filenames) > 0 else None
    if filename is None:
...
try:
    ...
    def download_file(dest_dir, response):
        filename = get_filename_from_response(dest_dir, response)
...
def refresh_access_token():
    ...
    if response.status_code != 200:
        logging.error('response status :' + str(response.status_code) + ', ' + str(responses[response.status_code]))
    response_json = json.loads(response.text)
    return response_json['access_token']
...
def perform_rest_request(rtype, url, headers=None, params=None, files=None, stream=None):
    ...
    if rtype == 'get':
        ...
        response = requests.post(url, headers=headers, params=params, files=files, stream=stream, proxies=proxies, verify=verify, timeout=args.timeout)
    else:
...
def rest_request(rtype, url, params=None, files=None, stream=None, raise_for_status=True):
    global access_token
...
def download_dataset(dest_dir, dataset_id, file_format):
    ...
    download_file(dest_dir, response)
    return
...
try:
    ...
    if dataset_ids:
        with open(dataset_ids) as file:
            ...
            download_datasets(dest_dir, dataset_id_list, file_format)
