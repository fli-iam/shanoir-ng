...
def ask_access_token():
    ...
    response = requests.post(url, data=payload, headers=headers, proxies=proxies, verify=verify, timeout=20)
...
def perform_rest_request(rtype, url, headers=None, params=None, files=None, stream=None):
    ...
    if rtype == 'get':
        response = requests.get(url, headers=headers, params=params, stream=stream, proxies=proxies, verify=verify, timeout=20)
    elif rtype == 'post':
        response = requests.post(url, headers=headers, params=params, files=files, stream=stream, proxies=proxies, verify=verify, timeout=20)
    else:
