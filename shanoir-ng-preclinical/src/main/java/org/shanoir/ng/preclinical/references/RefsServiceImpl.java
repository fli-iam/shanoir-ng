...
public class RefsServiceImpl implements RefsService {
	...
	@Override
	public Reference save(final Reference reference) throws ShanoirPreclinicalException {
		Reference savedRef = null;
		try {
			savedRef = refsRepository.save(reference);
		} catch (DataIntegrityViolationException dive) {
			ShanoirPreclinicalException.logAndThrow(LOG, "Error while creating reference: " + dive.getMessage());
		}
		return savedRef;
	}

	@Override
	public Reference update(final Reference ref) throws ShanoirPreclinicalException {
		final Reference refDb = refsRepository.findOne(ref.getId());
		updateReferenceValues(refDb, ref);
		try {
			refsRepository.save(refDb);
		} catch (Exception e) {
			ShanoirPreclinicalException.logAndThrow(LOG, "Error while updating reference: " + e.getMessage());
		}
		return refDb;
	}
